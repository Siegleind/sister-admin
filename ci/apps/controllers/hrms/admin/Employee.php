 ...
 * @author NaYeM
 */
class Employee extends SISTER_Controller {

    public function __construct() {
        parent::__construct('HRMS');
        $this->load->library(['Session','menu', 'breadcrumbs','language']);
        ...
        if ($user_type != 1) {
            ...
            $exception_uris = array(
                'hrms/admin/user',
                'hrms/admin/user/user_list'
            );
            if (in_array($this->uri->uri_string(), $exception_uris) == TRUE) {
                redirect('hrms/admin/dashboard');
            }
        }
        $this->load->model('hrms/employee_model');
    }
    ...
    public function add_employee($id = NULL) {
        ...
        if (!empty($id)) {// retrive data from db by id            
            ...
            if (empty($data['employee_info'])) {
                ...
                redirect('hrms/admin/employee/add_employee');
            }
        }
        
        $all_dept_info = $this->employee_model->get_employee_department();
        ...
        $data['all_country'] = $this->employee_model->get_all_country();
        
        //page load
        $data['subview'] = $this->load->view('hrms/admin/employee/add_employee', $data, TRUE);
        
        $this->load->view('hrms/admin/_layout_main', $data);
    }
    ...
    public function save_employee($id = NULL) {
        ...
        if (!empty($_FILES['resume']['name'])) {
            ...
            $val == TRUE || redirect('hrms/admin/employee/add_employee');
        ...
        set_message($type, $message);
        redirect('hrms/admin/employee/employee_list'); //redirect page
    }
    ...
    public function employee_list($id = NULL) {
        $this->load->helper(['dompdf','admin']);
        $data['title'] = "Employee List";
        $data['all_employee_info'] = $this->employee_model->all_emplyee_info();
        $data['subview'] = $this->load->view('hrms/admin/employee/employee_list', $data, TRUE);
        $this->load->view('hrms/admin/_layout_main', $data);
    }
    ...
    public function employee_list_pdf() {
        ...
        $view_file = $this->load->view('hrms/admin/employee/employee_list_pdf', $data, true);
    ...
    public function view_employee($id = NULL) {
        ...
        if (!empty($id)) {// retrive data from db by id
            ...
            if (empty($data['employee_info'])) {
                ...
                $message = "No Record Found";
                set_message($type, $message);
                redirect('hrms/admin/employee/employee_list');
            }
        }
        $data['subview'] = $this->load->view('hrms/admin/employee/view_employee', $data, TRUE);
        $this->load->view('hrms/admin/_layout_main', $data);
    }
    ...
    public function save_employee_award($id = NULL) {
        ...
        set_message($type, $message);
        redirect('hrms/admin/employee/employee_award'); //redirect page
    }
    ...
    public function make_pdf($employee_id) {
        ...
        $this->load->helper('dompdf');
        $view_file = $this->load->view('hrms/admin/employee/employee_view_pdf', $data, true);
